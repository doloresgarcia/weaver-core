...
class LapPENodeEncoder(torch.nn.Module):
    ...
    def __init__(self, dim_emb, expand_x=True):
        super().__init__()
        dim_in = 35  # cfg.share.dim_in  # Expected original input node features dim
        print("dim in FOR LAPLACIAN NODE ENC", dim_in)
        pecfg = cfg.posenc_LapPE
        dim_pe = pecfg.dim_pe  # Size of Laplace PE embedding
        print("dim in FOR LAPLACIAN embedding ", dim_pe)
